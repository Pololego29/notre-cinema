...
import { UPLOAD_CATEGORIES } from '../config'
...
const INPUT_CLASS =
  'w-full bg-transparent border-b font-body text-sm text-white placeholder-white/25 outline-none py-2 transition-colors duration-200 focus:border-gold/60'
...
export default function MemoryUploadModal({ onClose, onUploaded, profile }) {
  ...
  const handleFile = useCallback((f) => {
    ...
    if (!validTypes.includes(f.type)) {
      setError('Format non supporté. Utilise JPG, PNG, WEBP, GIF, MP4, MOV ou WEBM.')
      return
    }
    setError('')
    setFile(f)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setTimeout(() => {
        setPhase('success')
        setTimeout(() => onUploaded(memory), 1200)
      }, 300)
    } catch {
      clearInterval(tick)
      setPhase('form')
...
function FormState({
  form, setField, file, isVideo, previewUrl, isDragging, error,
  fileInputRef, thumbInputRef, onDrop, onDragOver, onDragLeave,
  handleFile, setThumbnailFile, onClose, handleSubmit,
}) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
    ...
    >
      ...
      >
        <div>
          ...
          <p className="text-white/30 text-xs font-body mt-0.5">Capture un moment pour toujours</p>
        </div>
        <button
          onClick={onClose}
      ...
      <form onSubmit={handleSubmit} className="px-6 py-5 flex flex-col gap-5">
        ...
        <div
          ...
          style={{
            minHeight: 160,
            border: `1.5px dashed ${isDragging ? 'rgba(212,175,55,0.6)' : 'rgba(255,255,255,0.12)'}`,
        ...
        >
          {previewUrl ? (
            <div className="relative w-full" style={{ aspectRatio: '16/9' }}>
              ...
              {isVideo && (
                <div
                  className="absolute top-2 right-2 font-body text-xs px-2 py-1 rounded"
            ...
            <div className="flex flex-col items-center justify-center gap-3 py-10 text-white/25">
              ...
              <div className="text-center">
                ...
                <p className="font-body text-xs mt-1 opacity-60">ou clique pour parcourir</p>
                <p className="font-body text-xs mt-2 opacity-40">JPG · PNG · WEBP · MP4 · MOV · WEBM</p>
              </div>
            </div>
          )}
        </div>
        ...
        {isVideo && (
          <div>
            ...
            <button
              ...
              className="font-body text-xs text-white/40 hover:text-white/70 border rounded px-3 py-1.5 transition-colors duration-200"
        ...
        <div className="flex flex-col gap-5">
          {/* Titre */}
          <div>
            <label className="font-body text-xs text-white/40 tracking-wider uppercase block mb-1.5">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setField('memory_date', e.target.value)}
                className={INPUT_CLASS}
        ...
        {/* Séparateur */}
        <div style={{ height: 1, background: 'linear-gradient(90deg, rgba(212,175,55,0.15), transparent)' }} />

        {/* Bouton submit */}
        <motion.button
          type="submit"
          className="w-full py-3 font-body text-sm tracking-widest uppercase text-white rounded-lg transition-all duration-200"
...
function UploadingState({ progress }) {
  return (
    ...
    >
      ...
      <div className="w-full rounded-full overflow-hidden" style={{ height: 2, background: 'rgba(255,255,255,0.08)' }}>
        <motion.div
      ...
      <p className="font-body text-xs text-white/30">{Math.round(Math.min(progress, 100))} %</p>
    </motion.div>
  )
}
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        ...
        <p className="font-body text-xs text-white/35 mt-1">Il apparaît maintenant dans la bonne catégorie</p>
      </div>
    </motion.div>
  )
}
